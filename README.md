# Multi-Platform Auto-Apply Suite (Wellfound + Instahyre + Foundit)

A unified, precision-first automated job application suite for **Wellfound** (formerly AngelList Talent), **Instahyre** (`instahyre.com`), and **Foundit** (`foundit.in`, formerly Monster India).
...
[Extract Unique Job/Opportunity ID]
   ...
   │   ├── 4. Experience Limit? (4+, 5+, 2-5 yrs, 3-5 yrs, 3-6 yrs) ─► ❌ SKIP: EXPERIENCE_EXCEEDS_LIMIT
...
- ❌ **Hard Rejected:** `4+ yrs`, `5+ yrs`, and senior ranges like `2–5 years`, `3–5 years`, `3–6 years`, `4–7 years`.
...
Fill in your details in `.env` once. All platforms (Wellfound, Instahyre, and Foundit) use the exact same configuration!
...
npm run dry-run:wellfound    # Step 2: Safe dry run (evaluates scores, simulates apply)
...
npm run dry-run:instahyre    # Step 2: Safe dry run (evaluates scores, fills questionnaires)
...
npm run dry-run:foundit      # Step 2: Safe dry run (scans search queries, tests modals)
...
1. **Remove Senior Titles from Hard Exclusions:**
   In `wellfound-auto-apply.js`, `instahyre-auto-apply.js`, and `foundit-auto-apply.js`, remove `/\bsenior\b/`, `/\blead\b/`, `/\bprincipal\b/`, and `/\bstaff\b/` from `TITLE_HARD_EXCLUSIONS`.
...
| [`applied-jobs-foundit.json`](file:///d:/MY-PROJECTS/Wellfound_AutoApply_WorkFlow/applied-jobs-foundit.json) | Anti-duplicate database for Foundit. |
