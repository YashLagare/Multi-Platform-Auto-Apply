...
${CV.name}`;
  ...
  const log = (...a) =>
    console.log("%c[auto-apply]", "color:#0a84ff;font-weight:bold", ...a);
  ...
  function isJobAlreadyApplied(jobId, url) {
    ...
    try {
      ...
      const history = JSON.parse(
        localStorage.getItem("wf_applied_history") || "{}",
      );
  ...
  function cleanTitle(raw) {
    return (raw || "")
      .replace(/\s+/g, " ")
      .split(
        /remote only|on-?site|hybrid|₹|\$\d|€|posted \d|recruiter|•|\d+\s?(?:weeks?|days?|months?|hours?)\s?ago/i,
      )[0]
  ...
  function findButtonByText(root, regex) {
    return [
      ...root.querySelectorAll('button, a[role="button"], [type="submit"]'),
  ...
  function checkExperienceExclusion(jdText) {
    ...
    const plusMatch = t.match(
      /\b(?:[4-9]|1\d)\+?\s*(?:years?|yrs?)(?:\s+of\s+experience|\s+experience)?\b/i,
    );
    if (plusMatch && !/\b(?:0-2|0-3|1-3|1-2)\b/.test(plusMatch[0])) {
      ...
      if (num >= 4) {
        return {
          pass: false,
          reason: `EXPERIENCE_EXCEEDS_LIMIT (${plusMatch[0]})`,
        };
      }
    }
    ...
    const minMatch = t.match(
      /\b(?:min(?:imum)?|at\s*least)\s*of?\s*([4-9]|1\d)\s*(?:years?|yrs?)\b/i,
    );
    if (minMatch) {
      return {
        pass: false,
        reason: `EXPERIENCE_EXCEEDS_LIMIT (${minMatch[0]})`,
      };
    }
  ...
  function checkLocationFit(cardLoc, jdText) {
    ...
    const isRemote =
      /\bremote\b|\bwork\s*from\s*home\b|\bwfh\b|\banywhere\b/i.test(combined);

    // Foreign residency locks (e.g. US/UK/EU citizens only)
    if (
      /\b(?:must\s+reside\s+in\s+the\s+u\.?s\.?|us\s+citizens\s+only|us\s+only|uk\s+only|eu\s+only|north\s+america\s+only)\b/i.test(
        combined,
      )
    ) {
    ...
    const p1 = /\bbengaluru\b|\bbangalore\b|\bpune\b|\bhyderabad\b/i.test(
      combined,
    );
    const p2 =
  ...
  function calculateMatchScore(job, jdText) {
    ...
    let techPoints = 0;
    if (/\breact(?:\.js|js)?\b/.test(lowerJD)) {
      techPoints += 10;
      breakdown.push("React (+10)");
    }
    if (/\bnext(?:\.js|js)?\b/.test(lowerJD)) {
      techPoints += 8;
      breakdown.push("Next.js (+8)");
    }
    if (/\btypescript\b/.test(lowerJD)) {
      techPoints += 7;
      breakdown.push("TypeScript (+7)");
    } else if (/\bjavascript\b|\bes6\b/.test(lowerJD)) {
      techPoints += 5;
      breakdown.push("JavaScript (+5)");
    }

    if (/\bnode(?:\.js|js)?\b|\bexpress(?:\.js)?\b|\bmongodb\b/.test(lowerJD)) {
      techPoints += 5;
    ...
    if (
      /\b(?:0-1|0-2|0-3|1-2|1-3|fresh|fresher|entry\s*level|associate|junior)\b/.test(
        lowerJD,
      )
    ) {
      score += 20;
    ...
    if (
      /bengaluru|bangalore|pune|hyderabad/.test(loc) ||
      /remote|work from home/.test(loc)
    ) {
      score += 10;
  ...
  function getCompany() {
    ...
    let name = (el?.textContent || "")
      ...
      .trim();
    if (
      /about the job|about us|apply|jobs|follow|save|^$/i.test(name) ||
      name.split(" ").length > 6
    )
      name = "";
    return name;
  ...
  async function fillAndSubmit(company, title) {
    const modal = await waitFor(() => {
      ...
      return (
        dialogs.find(
          (d) =>
            /apply to /i.test(d.textContent) && d.querySelector("textarea"),
        ) ||
    ...
    const extraFields = [
      ...textareas.slice(1),
      ...[...scope.querySelectorAll('input[type="text"]:not([value])')].filter(
        visible,
      ),
    ];
    ...
    if (
      /does not support the locations|update your location preferences/i.test(
        scope.textContent,
      )
    ) {
      const relocateChoice = [
        ...scope.querySelectorAll('label, [role="radio"], button, div'),
      ]
        .filter(visible)
        .find(
          (el) =>
            /i can relocate/i.test(el.textContent) &&
      ...
      if (relocateChoice) {
        ...
        await sleep(800);
        const nativeSel = [...scope.querySelectorAll("select")]
          .filter(visible)
          .pop();
        if (nativeSel) {
          const opts = [...nativeSel.options].filter(
    ...
    for (const sel of [...scope.querySelectorAll("select")].filter(visible)) {
      const opts = [...sel.options].filter(
        (o) => o.value && !/^select|^choose|^--/i.test(o.text.trim()),
      );
    ...
    const radioGroups = {};
    for (const r of [...scope.querySelectorAll('input[type="radio"]')].filter(
      visible,
    )) {
    ...
    for (const cb of [
      ...
      if (
        !cb.checked &&
        /relocat|agree|confirm|authoriz|terms|acknowledge|remote/i.test(own)
      ) {
        cb.click();
      }
    }
    ...
    const sendBtn = await waitFor(
      () => findButtonByText(scope, /^apply$|^send$|submit|send application/i),
      12000,
    );
    if (!sendBtn) {
  ...
  function findJobRows() {
    ...
    for (const a of document.querySelectorAll('a[href*="/jobs/"]')) {
      ...
      for (let i = 0; i < 5 && row && row.textContent.trim().length < 60; i++)
      ...
      if (
        /applied/i.test(
          [...row.querySelectorAll("button, span")]
            .map((e) => e.textContent.trim())
            .find((t) => /^applied$/i.test(t)) || "",
        )
      ) {
        continue;
      }
      ...
      // Freshness check
      const posted = row.textContent.match(
        /posted (?:about )?(\d+)\+? ?(day|week|month|hour|minute)s? ago/i,
      );
      let postedHours = 12;
      let postedDays = 1;
      if (posted) {
        ...
        if (unit.startsWith("hour") || unit.startsWith("minute")) {
          postedHours = n;
          postedDays = 0;
      ...
      const company = (row.querySelector('img[alt*="logo" i]')?.alt || "")
        ...
        .trim();
      const salary = (row.textContent.match(
        /(?:₹|\$|€)\s?[\d.,k]+\s?(?:[–-]\s?(?:₹|\$|€)?\s?[\d.,k]+)?k?/i,
      ) || [""])[0].trim();
      const location = (row.textContent.match(
        /(?:in-office|remote|hybrid|in office)[^•\n]{0,60}/i,
      ) || [""])[0].trim();
  ...
  let applied = 0;
  log(
    `Starting Precision Two-Layer Apply. DRY_RUN=${CONFIG.DRY_RUN}, max=${CONFIG.MAX_APPLICATIONS}, Score Threshold=${CONFIG.SCORE_THRESHOLD}`,
  );
  ...
  while (applied < CONFIG.MAX_APPLICATIONS) {
    ...
    if (!allRows.length) {
      log(
        "No un-applied job cards found on current feed. Scrolling to load more...",
      );
      let grew = false;
    ...
    log(
      `▶ Evaluating: ${job.title} @ ${job.company || "?"} | ${job.href} | Job ID: ${job.id}`,
    );
    ...
    if (!jdText || jdText.length < 80) {
      log(
        `🚫 [SKIP: INSUFFICIENT_JD_DATA] Job ID: ${job.id} (JD length: ${jdText.length} chars)`,
      );
      appliedJobIds[job.id] = true;
      (
        document.querySelector(
          'button[aria-label="Close" i], [class*="Modal" i] button[class*="close" i]',
    ...
    log(
      `  📊 Evaluation Score: ${matchResult.score}/100 [${matchResult.action}]`,
    );
