...
(async function naukriAutoApply() {
  ...
  const log = (...a) =>
    console.log("%c[naukri-apply]", "color:#0284c7;font-weight:bold", ...a);

  function extractJobId(el, href) {
    const fromIdAttr =
      el.getAttribute("data-job-id") ||
      el.getAttribute("data-jobid") ||
      el.getAttribute("data-tuple-id");
  ...
  function isJobAlreadyApplied(jobId) {
    ...
    try {
      ...
      const history = JSON.parse(
        localStorage.getItem("nk_applied_history") || "{}",
      );
  ...
  function findButtonByText(root, regex) {
    return [
      ...root.querySelectorAll(
        'button, a, div[role="button"], span[role="button"], [type="submit"], .btn, [class*="btn" i], [class*="button" i], [class*="save" i], [class*="send" i], [class*="apply" i]',
      ),
  ...
  function isExternalRedirect(el) {
    ...
    return /apply on (?:company|employer) site|external site|redirect/i.test(
      text,
    );
  }
  ...
  async function handleNaukriChatbotOrModal(job) {
    const container = await waitFor(
      () =>
        document.querySelector(
          '.chatbot_Wrapper, .drawer-wrapper, .modal-wrapper, [class*="chatbot" i], [class*="drawer" i]',
        ),
      3000,
    );
    if (!container) {
    ...
    const radioButtons = [
      ...container.querySelectorAll(
        'input[type="radio"], .option, .radio-option, [role="radio"]',
      ),
    ].filter(visible);
    for (const rb of radioButtons) {
      const txt = (
        rb.textContent ||
        rb.value ||
        rb.nextElementSibling?.textContent ||
        ""
      ).toLowerCase();
    ...
    // 4. Textareas
    const textareas = [...container.querySelectorAll("textarea")].filter(
      visible,
    );
    ...
    const submitBtn =
      findButtonByText(
        container,
        /apply|submit|send|save|confirm|next|continue/i,
      ) ||
      findButtonByText(
        document,
        /apply|submit|send|save|confirm|next|continue/i,
      ...
      container.querySelector(
        '.send-btn, .save-btn, .saveBtn, .save-drawer, button[type="submit"], input[type="submit"], .btn-primary, [class*="save" i], [class*="submit" i], [class*="send" i], [class*="apply" i]',
      ) ||
      document.querySelector(
        '.chatbot_Wrapper button, .drawer-wrapper button, [class*="chatbot" i] button, [class*="drawer" i] button, .save-btn, .saveBtn, .btn-primary',
      );

    if (!submitBtn) {
      log("  ⚠ No Submit button found in chatbot — closing");
      try {
        findButtonByText(container, /close|cancel|×/i)?.click() ||
    ...
    if (CONFIG.DRY_RUN) {
      log(
        ...
        (submitBtn.textContent || submitBtn.value || "").trim(),
      );
      try {
        findButtonByText(container, /close|cancel|×/i)?.click() ||
          findButtonByText(document, /close|cancel|×/i)?.click();
  ...
  function findNaukriCards() {
    ...
    for (const el of cardElements) {
      ...
      const linkEl = el.querySelector(
        'a.title, a[href*="/job-listings-"], a[href*="-job-"], a[href*="naukri.com/job"], h2 a, h3 a, a[class*="title" i]',
      );
      ...
      const title = (
        linkEl?.textContent ||
        el.querySelector('.title, h2, h3, [class*="title" i]')?.textContent ||
        ""
      ).trim();
      ...
      const company = (
        el.querySelector(
          '.comp-name, .subTitle, a.comp-name, [class*="company" i], [class*="comp-name" i]',
        )?.textContent || ""
      ).trim();

      const expEl = el.querySelector(
        '.expwdth, .experience, [class*="exp" i], .exp',
      );
      const experience = (
        expEl?.textContent ||
      ...
      const locEl = el.querySelector(
        '.locWdth, .location, [class*="location" i], .loc',
      );
      const location = (
        locEl?.textContent ||
      ...
      const skillsEl = el.querySelector(
        '.row5, .tag-li, .tags-gt, [class*="skill" i]',
      );
      ...
      const applyBtn =
        findButtonByText(el, /^apply$|^apply on company site$/i) ||
        el.querySelector('.apply-button, .apply-btn, [class*="apply" i]');
  ...
  const IS_SINGLE_JOB_PAGE =
    /naukri\.com\/(?:job-listings|job\/)/i.test(window.location.href) ||
    !!document.querySelector(
      'h1.jd-header-title, [class*="jd-header-title" i], #job-desc',
    );
  ...
  if (IS_SINGLE_JOB_PAGE) {
    ...
    const titleEl = document.querySelector(
      'h1.jd-header-title, h1, [class*="jd-header-title" i]',
    );
    const title = (titleEl?.textContent || document.title || "").trim();

    const companyEl = document.querySelector(
      '.jd-header-comp-name, a.pad-rt-8, [class*="comp-name" i]',
    );
    ...
    const experience = (
      expEl?.textContent ||
      document.body.textContent.match(/\d+\s*-\s*\d+\s*Yrs/i)?.[0] ||
      ""
    ).trim();

    const locEl = document.querySelector(
      '.loc, .location, [class*="location" i]',
    );
    const location = (
      locEl?.textContent ||
    ...
    const skillsEl = document.querySelector(
      '.key-skill, .skills, [class*="key-skill" i]',
    );
    const skills = (skillsEl?.textContent || document.body.textContent)
      .replace(/\s+/g, " ")
      .trim();
    ...
    if (!titleCheck.pass) {
      log(`🚫 [SKIP: ${titleCheck.reason}] Job ID: ${jobId} ("${title}")`);
      return;
    }

    const expCheck = checkExperienceExclusion(experience || job.description);
    if (!expCheck.pass) {
      log(`🚫 [SKIP: ${expCheck.reason}] Job ID: ${jobId} ("${title}")`);
      return;
    }

    const locCheck = checkLocationFit(location || job.description);
    if (!locCheck.pass) {
      log(`🚫 [SKIP: ${locCheck.reason}] Job ID: ${jobId} ("${title}")`);
      return;
    }
    ...
    log(
      `  🎯 MATCH APPROVED (${matchResult.score}/100) — Applying to ${company || "Company"}`,
    );
    const applyBtn = await waitFor(() => {
      return (
        findButtonByText(document, /apply/i) ||
        document.querySelector(
          '#apply-button, .apply-button, .apply-btn, [class*="apply" i], button.btn-primary',
        )
      );
    }, 4000);
  ...
  let applied = 0;
  log(
    `Starting Naukri Precision Two-Layer Apply. Mode=${CONFIG.DRY_RUN ? "DRY RUN" : "LIVE"}, Max=${CONFIG.MAX_APPLICATIONS}, Score Threshold=${CONFIG.SCORE_THRESHOLD}`,
  );
  ...
  while (applied < CONFIG.MAX_APPLICATIONS) {
    ...
    if (!cards.length) {
      log(
        "No un-applied job tuples found on current view. Scrolling down to trigger React lazy load...",
      );
    ...
    if (isJobAlreadyApplied(job.id)) {
      log(
        `🚫 [SKIP: ALREADY_APPLIED] Job ID: ${job.id} (${job.title} @ ${job.company})`,
      );
    ...
    if (isExternalRedirect(job.cardEl)) {
      log(
        `🚫 [SKIP: EXTERNAL_ATS_REDIRECT] Job ID: ${job.id} ("${job.title}")`,
      );
      appliedJobIds[job.id] = true;
      continue;
    }
    ...
    if (!expCheck.pass) {
      log(
        `🚫 [SKIP: ${expCheck.reason}] Job ID: ${job.id} ("${job.title}" - Exp: ${job.experience || "N/A"})`,
      );
      appliedJobIds[job.id] = true;
      continue;
    }
    ...
    if (!locCheck.pass) {
      log(
        `🚫 [SKIP: ${locCheck.reason}] Job ID: ${job.id} (Location: ${job.location || "Unspecified"})`,
      );
      appliedJobIds[job.id] = true;
      continue;
    }
    ...
    if (!btn) {
      ...
      const clickTarget =
        job.cardEl.querySelector(
          'a.title, .title, h2, h3, a[href*="/job-listings-"]',
        ) || job.cardEl;
      log(
        `  ↗ Opening job detail page in new tab for evaluation & application...`,
      );
      clickTarget.click();
      appliedJobIds[job.id] = true;
      await sleep(2500);
