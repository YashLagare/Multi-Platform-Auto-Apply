/**
 * Naukri.com Auto-Apply — Precision Two-Layer Decision Pipeline
 * ==============================================================================
 ...
 * Handles Direct 1-Click Apply and Naukri Chatbot / Screening Questionnaire Modals with Google Gemini AI.
 */
(async function naukriAutoApply() {
  'use strict';

  if (window.__naukriRunning) return;
  window.__naukriRunning = true;
  ...
  const log = (...a) => console.log('%c[naukri-apply]', 'color:#0284c7;font-weight:bold', ...a);

  function extractJobId(el, href) {
    const fromIdAttr = el.getAttribute('data-job-id') || el.getAttribute('data-jobid') || el.getAttribute('data-tuple-id');
    if (fromIdAttr) return fromIdAttr;

    const fromHref = (href || '').match(/-(\d{6,14})(?:\?|$)/)?.[1] ||
                     (href || '').match(/jobId=(\d+)/)?.[1] ||
                     (href || '').match(/(\d{8,14})/)?.[1];
    if (fromHref) return fromHref;

    return el.id?.replace(/^[^\d]*/, '') || null;
  }
  ...
  function isJobAlreadyApplied(jobId) {
    ...
    try {
      if (localStorage.getItem(`nk_applied_${jobId}`) === 'true') return true;
      const history = JSON.parse(localStorage.getItem('nk_applied_history') || '{}');
  ...
  function markJobAsApplied(jobId, jobData) {
    if (!jobId) return;
    try {
      localStorage.setItem(`nk_applied_${jobId}`, 'true');
      const history = JSON.parse(localStorage.getItem('nk_applied_history') || '{}');
      history[jobId] = { date: new Date().toISOString(), title: jobData.title, company: jobData.company };
      localStorage.setItem('nk_applied_history', JSON.stringify(history));
    } catch (e) {}
  }
  ...
  function findButtonByText(root, regex) {
    return [...root.querySelectorAll('button, a, div[role="button"], span[role="button"], [type="submit"], .btn, [class*="btn" i], [class*="button" i], [class*="save" i], [class*="send" i], [class*="apply" i]')]
  ...
  function checkExperienceExclusion(expText) {
    ...
    // Check senior ranges (e.g. 2-5 yrs, 3-5 yrs, 3-6 yrs, 4-8 yrs, 5-10 yrs)
  ...
  function isExternalRedirect(el) {
    ...
    return /apply on (?:company|employer) site|external site|redirect/i.test(text);
  }
  ...
  async function answerQuestion(label) {
    if (!label || label.trim().length < 2) return GENERIC_ANSWER;
      ...
      try {
        ...
        const res = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${CONFIG.geminiKey}`,
          {
            ...
            body: JSON.stringify({
              contents: [{ parts: [{ text:
                `You are answering a screening question on Naukri.com on my behalf. Answer in first person, 1-3 sentences, professional, no markdown.\n\nMy CV:\n${JSON.stringify(CV)}\n\nQuestion: ${label}` }] }],
            }),
          }
        );
  ...
  async function handleNaukriChatbotOrModal(job) {
    const container = await waitFor(() => document.querySelector('.chatbot_Wrapper, .drawer-wrapper, .modal-wrapper, [class*="chatbot" i], [class*="drawer" i]'), 3000);
    if (!container) {
      log('  ✅ Direct 1-Click Apply submitted on Naukri');
      return true;
    }

    log('  📋 Answering Naukri Chatbot / Questionnaire modal...');

    // 1. Radio / Button choices in Chatbot (e.g. Notice period choices)
    const radioButtons = [...container.querySelectorAll('input[type="radio"], .option, .radio-option, [role="radio"]')].filter(visible);
    for (const rb of radioButtons) {
      const txt = (rb.textContent || rb.value || rb.nextElementSibling?.textContent || '').toLowerCase();
      if (/immediate|0|15|yes/i.test(txt)) {
        try { rb.click(); } catch (e) {}
        log('  ✍ Selected option:', txt.slice(0, 30));
        await sleep(400);
      }
    }

    // 2. Text / Number Inputs
    const inputs = [...container.querySelectorAll('input[type="text"], input[type="number"], input[type="tel"]')].filter(visible);
    ...
    // 5. Submit button (Search document-wide inside open drawer / modal)
    const submitBtn = findButtonByText(container, /apply|submit|send|save|confirm|next|continue/i) ||
                      findButtonByText(document, /apply|submit|send|save|confirm|next|continue/i) ||
                      container.querySelector('.send-btn, .save-btn, .saveBtn, .save-drawer, button[type="submit"], input[type="submit"], .btn-primary, [class*="save" i], [class*="submit" i], [class*="send" i], [class*="apply" i]') ||
                      document.querySelector('.chatbot_Wrapper button, .drawer-wrapper button, [class*="chatbot" i] button, [class*="drawer" i] button, .save-btn, .saveBtn, .btn-primary');

    if (!submitBtn) {
      log('  ⚠ No Submit button found in chatbot — closing');
      try { findButtonByText(container, /close|cancel|×/i)?.click() || findButtonByText(document, /close|cancel|×/i)?.click(); } catch (e) {}
      return false;
    }

    if (CONFIG.DRY_RUN) {
      log('  🔍 DRY_RUN — would click submit:', (submitBtn.textContent || submitBtn.value || '').trim());
      try { findButtonByText(container, /close|cancel|×/i)?.click() || findButtonByText(document, /close|cancel|×/i)?.click(); } catch (e) {}
      return true;
    }
    ...
    await sleep(1500);
    log('  ✅ Application submitted in Chatbot modal');
    return true;
  }

  // ======================= SCRAPE NAUKRI CARDS =======================
  function findNaukriCards() {
    const cards = [];
    const cardElements = document.querySelectorAll(
      '.srp-jobtuple-wrapper, article.jobTuple, div.cust-job-tuple, .tuple, [data-job-id], div[class*="jobTuple" i], article[class*="jobTuple" i], div[class*="srp-jobtuple" i], div[class*="jcard" i], div[class*="tuple" i], .jobTupleHeader'
    );

    for (const el of cardElements) {
      if (!visible(el)) continue;

      const linkEl = el.querySelector('a.title, a[href*="/job-listings-"], a[href*="-job-"], a[href*="naukri.com/job"], h2 a, h3 a, a[class*="title" i]');
      const href = linkEl?.getAttribute('href') || '';
      const jobId = extractJobId(el, href);
      if (!jobId || isJobAlreadyApplied(jobId)) continue;

      const title = (linkEl?.textContent || el.querySelector('.title, h2, h3, [class*="title" i]')?.textContent || '').trim();
      if (!title || title.length < 3) continue;

      const company = (el.querySelector('.comp-name, .subTitle, a.comp-name, [class*="company" i], [class*="comp-name" i]')?.textContent || '').trim();

      const expEl = el.querySelector('.expwdth, .experience, [class*="exp" i], .exp');
      const experience = (expEl?.textContent || el.textContent.match(/\d+\s*-\s*\d+\s*Yrs/i)?.[0] || '').trim();

      const locEl = el.querySelector('.locWdth, .location, [class*="location" i], .loc');
      ...
      const skillsEl = el.querySelector('.row5, .tag-li, .tags-gt, [class*="skill" i]');
      const skills = (skillsEl?.textContent || el.textContent).replace(/\s+/g, ' ').trim();

      const applyBtn = findButtonByText(el, /^apply$|^apply on company site$/i) || el.querySelector('.apply-button, .apply-btn, [class*="apply" i]');
  ...
  const IS_SINGLE_JOB_PAGE = /naukri\.com\/(?:job-listings|job\/)/i.test(window.location.href) ||
                             !!document.querySelector('h1.jd-header-title, [class*="jd-header-title" i], #job-desc');

  if (IS_SINGLE_JOB_PAGE) {
    log(`📄 Single Job Detail Page detected: ${window.location.href}`);
    await sleep(2500);

    const titleEl = document.querySelector('h1.jd-header-title, h1, [class*="jd-header-title" i]');
    const title = (titleEl?.textContent || document.title || '').trim();

    const companyEl = document.querySelector('.jd-header-comp-name, a.pad-rt-8, [class*="comp-name" i]');
    const company = (companyEl?.textContent || '').trim();

    const expEl = document.querySelector('.exp, .experience, [class*="exp" i]');
    const experience = (expEl?.textContent || document.body.textContent.match(/\d+\s*-\s*\d+\s*Yrs/i)?.[0] || '').trim();

    const locEl = document.querySelector('.loc, .location, [class*="location" i]');
    const location = (locEl?.textContent || document.body.textContent.match(/Bengaluru|Bangalore|Pune|Hyderabad|Remote|Mumbai|Gurgaon|Noida|Delhi|Ahmedabad/i)?.[0] || '').trim();

    const skillsEl = document.querySelector('.key-skill, .skills, [class*="key-skill" i]');
    const skills = (skillsEl?.textContent || document.body.textContent).replace(/\s+/g, ' ').trim();

    const href = window.location.href;
    const jobId = extractJobId(document.body, href);
    const job = { id: jobId, href, title, company, experience, location, skills, description: document.body.textContent };

    if (!jobId || isJobAlreadyApplied(jobId)) {
      log(`🚫 [SKIP: ALREADY_APPLIED] Job ID: ${jobId || 'N/A'}`);
      return;
    }

    const titleCheck = checkTitleHardExclusions(title);
    if (!titleCheck.pass) { log(`🚫 [SKIP: ${titleCheck.reason}] Job ID: ${jobId} ("${title}")`); return; }

    const expCheck = checkExperienceExclusion(experience || job.description);
    if (!expCheck.pass) { log(`🚫 [SKIP: ${expCheck.reason}] Job ID: ${jobId} ("${title}")`); return; }

    const locCheck = checkLocationFit(location || job.description);
    if (!locCheck.pass) { log(`🚫 [SKIP: ${locCheck.reason}] Job ID: ${jobId} ("${title}")`); return; }
    ...
    if (!matchResult.passed) {
      log(`🚫 [SKIP: LOW_MATCH_SCORE (${matchResult.score}/100)]`);
      await sleep(1000);
      try { window.close(); } catch (e) {}
      return;
    }

    log(`  🎯 MATCH APPROVED (${matchResult.score}/100) — Applying to ${company || 'Company'}`);
    const applyBtn = await waitFor(() => {
      return findButtonByText(document, /apply/i) ||
             document.querySelector('#apply-button, .apply-button, .apply-btn, [class*="apply" i], button.btn-primary');
    }, 4000);

    if (!applyBtn) {
      log('  ⚠ No Apply button found on single job page');
      await sleep(1000);
      try { window.close(); } catch (e) {}
      return;
    }
    ...
    applyBtn.click();
    await sleep(2000);
    const ok = await handleNaukriChatbotOrModal(job);
    if (ok) {
      appliedJobIds[jobId] = true;
      markJobAsApplied(jobId, job);
    }
    await sleep(1500);
    try { window.close(); } catch (e) {}
    return;
  ...
  let applied = 0;
  log(`Starting Naukri Precision Two-Layer Apply. Mode=${CONFIG.DRY_RUN ? 'DRY RUN' : 'LIVE'}, Max=${CONFIG.MAX_APPLICATIONS}, Score Threshold=${CONFIG.SCORE_THRESHOLD}`);
  ...
  while (applied < CONFIG.MAX_APPLICATIONS) {
    const cards = findNaukriCards();

    if (!cards.length) {
      log('No un-applied job tuples found on current view. Scrolling down to trigger React lazy load...');
      window.scrollBy(0, 800);
      await sleep(3000);
      window.scrollBy(0, 800);
      await sleep(3000);
      const newCards = findNaukriCards();
      if (!newCards.length) {
        log('Naukri search results exhausted for current query.');
        break;
      }
      continue;
    }
    ...
    if (!btn) {
      // Click card title to open job detail page in a new tab
      const clickTarget = job.cardEl.querySelector('a.title, .title, h2, h3, a[href*="/job-listings-"]') || job.cardEl;
      log(`  ↗ Opening job detail page in new tab for evaluation & application...`);
      clickTarget.click();
      appliedJobIds[job.id] = true;
      await sleep(2500);
      continue;
  ...
  log(`Finished. Total processed: ${applied} Naukri applications.`);
})();
