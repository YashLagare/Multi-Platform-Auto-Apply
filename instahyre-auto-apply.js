/**
 * Instahyre Auto-Apply — Precision Two-Layer Decision Pipeline
 * ==============================================================
...
(async function instahyreAutoApply() {
  ...
  while (applied < CONFIG.MAX_APPLICATIONS) {
    ...
    const ok = await handleApplyModal(job);
    if (ok) {
      applied++;
      appliedJobIds[job.id] = true;
  ...
  log(`Finished. Total processed: ${applied} Instahyre applications.`);
})();
