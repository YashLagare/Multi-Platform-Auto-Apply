...
const CV = {
  ...
  currentCTC: g('CURRENT_CTC').replace(/\s*lpa$/i, '').trim(),                 // bare number for chatbots, e.g. "3"
  expectedCTC: g('EXPECTED_CTC').replace(/\s*lpa$/i, '').trim(),               // e.g. "4.5"
  currentSalary: g('CURRENT_CTC').replace(/\s*lpa$/i, '').trim() ? g('CURRENT_CTC').replace(/\s*lpa$/i, '').trim() + ' LPA' : '',
  expectedSalary: g('EXPECTED_CTC').replace(/\s*lpa$/i, '').trim() ? g('EXPECTED_CTC').replace(/\s*lpa$/i, '').trim() + ' LPA' : '',
  dob: g('DOB'),
  gender: g('GENDER'),
  ...
  startDate: g('NOTICE_PERIOD').toLowerCase().startsWith('immediat')
