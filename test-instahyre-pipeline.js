...
function evaluateInstahyreJob(job, appliedHistory = {}) {
  ...
  // Exp
  if (
    /\b(?:0\s*-\s*2|1\s*-\s*3|0\s*-\s*1|0\s*-\s*3)\s*yrs\b/.test(
...
const tests = [
  ...
  },
  {
    ...
    job: {
      ...
      skills: "Next.js, React.js, JavaScript, Node.js, Tailwind CSS",
    },
    expectedPass: true,
    minScore: 85,
  },
...
for (const tc of tests) {
  ...
  console.log(
    `  -> Pass: ${res.pass} | Score: ${res.score || "N/A"} | Reason: ${res.reason}`,
  );
  ...
  if (tc.expectedPass) {
    if (!res.pass || (tc.minScore && res.score < tc.minScore)) {
      console.error(
        `  ❌ FAILED: Expected pass with score >= ${tc.minScore}, got ${res.pass} (${res.score})`,
      );
      allPassed = false;
    } else {
      console.log(`  ✅ PASSED\n`);
    ...
    if (res.pass || !res.reason.startsWith(tc.expectedReasonPrefix)) {
      console.error(
        `  ❌ FAILED: Expected fail with ${tc.expectedReasonPrefix}, got ${res.reason}`,
      );
      allPassed = false;
    } else {
      console.log(`  ✅ PASSED\n`);
