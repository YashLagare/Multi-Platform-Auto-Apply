...
(async function founditAutoApply() {
  ...
  const log = (...a) =>
    console.log("%c[foundit-apply]", "color:#e11d48;font-weight:bold", ...a);
  ...
  function isJobAlreadyApplied(jobId) {
    ...
    try {
      ...
      const history = JSON.parse(
        localStorage.getItem("fi_applied_history") || "{}",
      );
  ...
  function findButtonByText(root, regex) {
    return [
      ...root.querySelectorAll(
        'button, a[role="button"], [type="submit"], .btn, .apply-btn, .btn-apply',
      ),
  ...
  function isExternalRedirect(el) {
    ...
    return /apply on (?:company|employer) website|external website|redirect/i.test(
      text,
    );
  }
              ...
              contents: [
                {
                  parts: [
                    {
                      text: `You are answering a job screening question on Foundit on my behalf. Answer in first person, 1-3 sentences, professional, no markdown.\n\nMy CV:\n${JSON.stringify(CV)}\n\nQuestion: ${label}`,
                    },
                  ],
                },
              ],
  ...
  async function handleFounditModal(job) {
    const modal = await waitFor(
      () =>
        document.querySelector(
          '.modal.show, .modal.in, [role="dialog"], .apply-modal, .questionnaire-popup',
        ),
      4000,
    );
    if (!modal) {
    ...
    const submitBtn = findButtonByText(
      modal,
      /^apply$|^submit$|^send$|^confirm$|^apply now$/i,
    );
    if (!submitBtn) {
  ...
  function findFounditCards() {
    ...
    for (const el of cardElements) {
      ...
      const linkEl = el.querySelector(
        'a[href*="/job/"], a[href*="jobId"], .jobTitle a, h3 a',
      );
      ...
      const title = (
        linkEl?.textContent ||
        el.querySelector(".jobTitle, h3, h2")?.textContent ||
        ""
      ).trim();
      if (!title || title.length < 3) continue;

      const company = (
        el.querySelector('.companyName, .company-name, [class*="company" i]')
          ?.textContent || ""
      ).trim();

      const expEl = el.querySelector(
        '.exp, .experience, [class*="experience" i], .job-exp',
      );
      const experience = (
        expEl?.textContent ||
      ...
      const locEl = el.querySelector(
        '.loc, .location, [class*="location" i], .job-loc',
      );
      const location = (
        locEl?.textContent ||
      ...
      const skillsEl = el.querySelector(
        '.skill, .skills, [class*="skill" i], .keySkills',
      );
      ...
      const applyBtn =
        findButtonByText(el, /^apply$|^quick apply$|^apply now$/i) ||
        el.querySelector(".btn-apply, .applyBtn");
  ...
  let applied = 0;
  log(
    `Starting Foundit Precision Two-Layer Apply. Mode=${CONFIG.DRY_RUN ? "DRY RUN" : "LIVE"}, Max=${CONFIG.MAX_APPLICATIONS}, Score Threshold=${CONFIG.SCORE_THRESHOLD}`,
  );
  ...
  while (applied < CONFIG.MAX_APPLICATIONS) {
    ...
    if (!btn) {
      ...
      const clickTarget =
        job.cardEl.querySelector("a, h3, h2, .jobTitle") || job.cardEl;
      clickTarget.click();
      await sleep(1800);
      btn =
        findButtonByText(document, /^apply$|^quick apply$|^apply now$/i) ||
        document.querySelector(
          '.btn-apply, .applyBtn, [class*="applyBtn" i], .apply-btn',
        );
    }
